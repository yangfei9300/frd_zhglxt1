<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tongjiList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="姓名" align="center" prop="linkUserName" />
      <el-table-column label="未联络" align="center" prop="reason0" />
      <el-table-column label="意向强烈" align="center" prop="reason1" />
      <el-table-column label="意向一般" align="center" prop="reason2" />
      <el-table-column label="无意向" align="center" prop="reason3" />
      <el-table-column label="无此人" align="center" prop="reason4" />
      <el-table-column label="无应答" align="center" prop="reason5" />
      <el-table-column label="号码无效 " align="center" prop="reason6" />
      <el-table-column label="需跟进" align="center" prop="reason7" />
      <el-table-column label="人不在 " align="center" prop="reason8" />
      <el-table-column label="同人 " align="center" prop="reason9" />
      <el-table-column label="非目标行业、区域" align="center" prop="reason10" />
      <el-table-column label="其他" align="center" prop="reason11" />
      <el-table-column label="合计" align="center" prop="reasonTotal" />
    </el-table>


<div class="roww center_center" style="padding:20px 0px;">
   <el-radio v-model="radio" @change="typeChange(1)" label="1">按数量显示</el-radio>
    <el-radio v-model="radio"  @change="typeChange(2)" label="2">按工作人员百分比显示	</el-radio>
    <el-radio v-model="radio"  @change="typeChange(3)" label="3">按类别的百分比显示	</el-radio>
</div>
    ...
    typeChange(res){
     ...
     // 获取任务列表
      ...
      getList() {
        ...
        if(this.tongjiType==1){
          // 按照数量统计
          callcountcount(queryParams).then(response => {
            console.log("统计结果",response)
              this.loading = false;
              this.tongjiList=response.data;
          });
        }if(this.tongjiType==2){

          staffcountcall(queryParams).then(response => {
              console.log("统计结果",response)
                this.loading = false;
                this.tongjiList=response.data;
            });
        }
        else if(this.tongjiType==3){
          // 按照百分比统计
          reasoncountcall(queryParams).then(response => {
            console.log("统计结果",response)
              this.loading = false;
              this.tongjiList=response.data;
          });
